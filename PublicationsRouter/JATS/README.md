...
| metadata.author.affiliations.identifier <br>metadata.author.affiliations.identifier.type <br>metadata.author.affiliations.identifier.id | `…<aff><institution-wrap><institution-id institution-id-type="…">` | The identifier object is created from `<institution-id institution-id-type="…">` element.<br>The `institution-id-type` **attribute** value converted to upper-case is used as the identifier `type`.<br>The `institution-id` element value is used as the identifier `id`. |
...
| metadata.author.affiliations.raw | `…<aff>` | This field will contain the text (excluding any subsidiary XML tags) found in the `<aff>` element |
...
| metadata.contributor.affiliations.identifier <br>metadata.author.affiliations.identifier.type <br>metadata.author.affiliations.identifier.id | `…<aff><institution-wrap><institution-id institution-id-type="…">` | The identifier object is created from `<institution-id institution-id-type="…">` element.<br>The `institution-id-type` **attribute** value converted to upper-case is used as the identifier `type`.<br>The `institution-id` element value is used as the identifier `id`. |
