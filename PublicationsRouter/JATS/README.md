...
| metadata.author.type | `<A-M><contrib-group><contrib contrib-type="…">` | `type` will be "author" unless a corresponding author is indicated via `<A-M><contrib-group><contrib><xref ref-type="corresp" rid="…x…">` in conjunction with `<A-M><author-notes><corresp id="…x…">` in which case it is set to "corresp".  |
...
| metadata.author.organisation_name | `<A-M><contrib-group><contrib><collab-name>`<br>or<br>`<A-M><contrib-group><contrib><collab-name-alternatives><collab-name>`<br>or<br>`<A-M><contrib-group><contrib><collab-wrap …><collab-name>`<br>or<br>`<A-M><contrib-group><contrib><collab>` (deprecated since JATS v1.4). | `<collab-name>`, `<collab-name-alternatives>` & `<collab-wrap>` were introduced in JATS v1.4, at which time `<collab>` was deprecated.<br><br> An organisation name will normally be present only if `<name>` is absent.  |
...
| metadata.author.affiliations.identifier.type<br><br>metadata.author.affiliations.identifier.id | `…<aff><institution-wrap><institution-id institution-id-type="…">`  | The `identifier` object is created from `<institution-id institution-id-type="…">` element.<br><br>`type` is derived from `institution-id-type` attribute; `id` is set to the element value.  |
| metadata.author.affiliations.org | `…<aff><institution>` <br>or<br> `…<aff><institution-wrap><institution>` |  |
| metadata.author.affiliations.dept | `…<aff><institution content-type="…">` <br>or<br> `…<aff><institution-wrap><institution content-type="…">`  | For an `<institution …>` element to be recognised as a `dept` field its `content-type` must contain one of: <ul><li>`dept`</li><li>`department`</li><li>`division`</li><li>`office`.</li></ul>Examples:<br> `content-type="org-division"` <br>or `content-type="office"`  |
...
| metadata.contributor.organisation_name | `<A-M><contrib-group><contrib><collab-name>`<br>or<br>`<A-M><contrib-group><contrib><collab-name-alternatives><collab-name>`<br>or<br>`<A-M><contrib-group><contrib><collab-wrap …><collab-name>`<br>or<br>`<A-M><contrib-group><contrib><collab>` (deprecated since JATS v1.4). | An organisation name will normally be present only if `<name>` is absent.  |
...
| metadata.contributor.affiliations.identifier.type<br><br>metadata.contributor.affiliations.identifier.id | `…<aff><institution-wrap><institution-id institution-id-type="…">`  | The `identifier` object is created from `<institution-id institution-id-type="…">` element.<br><br>`type` is derived from `institution-id-type` attribute; `id` is set to the element value.  |
| metadata.contributor.affiliations.org | `…<aff><institution>` <br>or<br> `…<aff><institution-wrap><institution>` |  |
| metadata.contributor.affiliations.dept | `…<aff><institution content-type="…">` <br>or<br> `…<aff><institution-wrap><institution content-type="…">`  | For an `<institution …>` element to be recognised as a `dept` field its `content-type` must contain one of: <ul><li>`dept`</li><li>`department`</li><li>`division`</li><li>`office`.</li></ul>Examples:<br> `content-type="org-division"` <br>or `content-type="office"`  |
...
| metadata.accepted_date | `<A-M><pub-history><event …>[<event-desc>]*<date date-type="accepted">`<br><br>Or from deprecated `<history>` elements:<br>`<A-M><history><date date-type="accepted">`<br>or<br>`<A-M><history><date pub-type="…x…">` where `…x…` is either "am" or "aam" | The alternate options are for JATS v1.3 or older.  |
...
| metadata.history_date.date_type | The *date_type* is obtained from the history `<date>` or `<pub-date>` element's *date-type* attribute.<br> `<A-M><pub-history><event …>[<event-desc>]*<date date-type="…">`<br> `<A-M><pub-history><event …>[<event-desc>]*<pub-date date-type="…">`<br>* Note `<event-desc>` is optional.<br><br>History dates will also be retrieved from deprecated JATS `<history>` elements:<br>`<A-M><history><date date-type="…">` | The `date-type` attribute value is used.<br><br>A publication date obtained from `<A-M><pub-date>` will also be added to the history dates.<br><br>The `<history>` element is deprecated since JATS v1.4.  |
| metadata.history_date.date | Derived from `<year>`, `<month>`, `<day>` elements nested within `<date>` or `<pub-date>`:<br>`<A-M><pub-history><event …>[<event-desc>]*<date …><year>`<br>`<A-M><pub-history><event …>[<event-desc>]*<date …><month>`<br>`<A-M><pub-history><event …>[<event-desc>]*<date …><day>`<br>or<br>`<A-M><pub-history><event …>[<event-desc>]*<pub-date …><year>`<br>`<A-M><pub-history><event …>[<event-desc>]*<pub-date …><month>`<br>`<A-M><pub-history><event …>[<event-desc>]*<pub-date …><day>`<br>* Note `<event-desc>` is optional.<br><br>Also from deprecated JATS `<history>` elements:<br>`<A-M><history><date …><year>`<br>`<A-M><history><date …><month>`<br>`<A-M><history><date …><day>` | The `<year>`, `<month>`, `<day>` values are concatenated into date with format `YYYY-MM-DD`.<br><br>The `<history>` element is deprecated since JATS v1.4.  |
