...
| metadata.author.affiliations.identifier.type <br>metadata.author.affiliations.identifier.id | `…<aff><institution-wrap><institution-id institution-id-type="…">` | The identifier object is created from `<institution-id institution-id-type="…">` element.<br><br>The `institution-id-type` **attribute** value converted to upper-case is used as the identifier `type`.<br><br>The `institution-id` element value is used as the identifier `id`. ||
...
| metadata.contributor.affiliations.identifier.type <br>metadata.author.affiliations.identifier.id | `…<aff><institution-wrap><institution-id institution-id-type="…">` | The identifier object is created from `<institution-id institution-id-type="…">` element.<br><br>The `institution-id-type` **attribute** value converted to upper-case is used as the identifier `type`.<br><br>The `institution-id` element value is used as the identifier `id`. |
