...
| Issue:<br> `«metadata.author.affiliations.org» may be overpopulated: "Faculty of Education, Manchester University" (contains "Faculty")` | Indicates that a supplied affiliation organisation name ***may*** contain more than just the name of the institution.  For example, it may also contain a department name or other address elements.<br><br>The validation code uses keywords, so it is possible that *false positives* arise where an institution name includes a keyword, such as *"Los Alamos National Laboratory"*  (which contains the keyword "laboratory") - in these cases the issue can be safely ignored.  <br><br>See below for JATS XML coding examples for separating elements of an affiliation. |
| Issue:<br> `In the «metadata.author» array, X of the Y affiliations (from Z authors) have «org» values that may contain more than the institution name. Affiliation address elements, including department & organisation names, should each be tagged separately` | Indicates that a number of supplied affiliation organisation names may contain more than just the name of the institution (these issues are also logged separately - see previous row). <br><br>Within JATS XML, the organisation name is supplied in the affiliation ***\<institution\>*** element; department names may also be supplied in the <institution> element, BUT they should additionally be tagged with the ***content-type*** attribute with one of these values: **"dept", "department", "office" or "division"** (to differentiate it from the institution name).<br><br>For example "Faculty of Education, Manchester University" should be encoded as shown in the first example below. |
|………………………………………………………………………||
